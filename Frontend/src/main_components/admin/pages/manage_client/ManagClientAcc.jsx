...
import ClientService from "../../../../features/clients_service.js"; // Import ClientService
...
const Clients = () => {
  ...
  const handleDeleteClient = async () => {
    try {
      if (selectedClient) {
        await ClientService.deleteClient(selectedClient.id); // Delete client
        fetchClients();
  ...
  const handleSaveClient = async (data) => {
    ...
    try {
      if (selectedClient) {
        await ClientService.updateClient(selectedClient.id, formData); // Update client
      } else {
        await ClientService.createClient(formData); // Create new client
      }
      fetchClients();
      setOpenDialog(false);
    } catch (error) {
  ...
  return (
    <Box m="20px">
      ...
      <Box sx={{ height: "calc(100vh - 250px)", width: "100%", overflowY: "auto", bgcolor: "background.default", borderRadius: "8px", boxShadow: "0px 2px 10px rgba(0, 0, 0, 0.1)" }}>
        <DataGrid
          rows={filteredClients} // Update to use filteredClients
          columns={columns}
          pageSize={10}
